...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        m2 = Movie(title='Oppenheimer',
                overview='WAR,WAR and more WAR.',
                release_date=datetime(23, 6, 27, tzinfo=timezone.utc),
                running_time=180,
                budget=800_000_000,
                tmbd_id=1090,
                revenue=0,
                poster_path='')
        m2.save()
        j1 = Job.objects.get(name='Actor')

        for name in ['Cillian Murphy',
                     'Emily Blunt',
                     'Florence Pugh']:
            b = Person.objects.create(name=name)
            MovieCredit.objects.create(person=b, movie=m2m, job=j1)
